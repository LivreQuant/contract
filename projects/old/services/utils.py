...
from projects.old.services.config import (
    ALGOD_TOKEN,
    ALGOD_SERVER,
    ALGOD_PORT,
    INDEXER_TOKEN,
...
def get_algod_client():
    ...
    return algokit_utils.AlgorandClient(
        {
            "algod": {"token": ALGOD_TOKEN, "url": f"{ALGOD_SERVER}:{ALGOD_PORT}"},
            "indexer": {
                "token": INDEXER_TOKEN,
...
def get_app_client(app_id: int, sender_mnemonic: str) -> AssetsContractClient:
...
def get_app_factory(deployer_mnemonic: str) -> AssetsContractFactory:
    ...
    return algorand.client.get_typed_app_factory(
        AssetsContractFactory, default_sender=deployer.address, default_signer=deployer
    )
