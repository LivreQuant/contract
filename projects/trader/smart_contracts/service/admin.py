...
class ContractAdmin:
    ...
    def create_contract(
        ...
        logger.info(f"Contract successfully created and initialized with app ID: {app_client.app_id}")
    ...
    def update_contract_params(self, app_id: int, new_parameters: Dict[str, Any]) -> None:
        """
    ...
    def delete_contract(self, app_id: int) -> None:
        ...
        try:
            ...
            logger.info("Make sure the contract status is set to INACTIVE before deletion")
            raise
    ...
    def get_contract_state(self, app_id: int) -> Dict[str, Any]:
        ...
        # Get global state
        global_state = self.algorand.algod.application_info(app_id)["params"]["global-state"]
...
def main():
    ...
    create_parser.add_argument("--parameters", required=True, help="JSON string of parameters")

    # Update params command
    update_params_parser = subparsers.add_parser("update-params", help="Update contract parameters")
    update_params_parser.add_argument("--app-id", required=True, type=int, help="Application ID")
    update_params_parser.add_argument("--parameters", required=True, help="JSON string of parameters")

    # Update status command
    update_status_parser = subparsers.add_parser("update-status", help="Update contract status")
    update_status_parser.add_argument("--app-id", required=True, type=int, help="Application ID")
    update_status_parser.add_argument("--status", required=True, choices=["ACTIVE", "INACTIVE"], help="New status")

    # Delete contract command
    delete_parser = subparsers.add_parser("delete", help="Delete a contract")
    delete_parser.add_argument("--app-id", required=True, type=int, help="Application ID")
