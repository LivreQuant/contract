...
@pytest.fixture()
def trader_app_client(
    algorand_client: AlgorandClient, deployer: SigningAccount
) -> TraderAppClient:
    factory = algorand_client.client.get_typed_app_factory(
        TraderAppFactory, default_sender=deployer.address
    )
...
def test_says_hello(trader_app_client: TraderAppClient) -> None:
    result = trader_app_client.send.hello(args=("World",))
    assert result.abi_return == "Hello, World"


def test_simulate_says_hello_with_correct_budget_consumed(
    trader_app_client: TraderAppClient,
) -> None:
    result = (
        trader_app_client.new_group()
        .hello(args=("World",))
        .hello(args=("Jane",))
        .simulate()
    )
