...
if not USER_MNEMONIC:
  raise ValueError("USER_MNEMONIC environment variable not set. Please check your .env file.")
...
def deploy_trader_contract():
  ...
  if not approval_program_path or not clear_program_path:
    ...
    if len(approval_files) >= 2:
      ...
      raise FileNotFoundError("Could not identify approval and clear program files")
  ...
  global_schema = algod.StateSchema(num_uints=0, num_byte_slices=5)  # Adjust based on your contract
  local_schema = algod.StateSchema(num_uints=0, num_byte_slices=3)  # Adjust based on your contract
